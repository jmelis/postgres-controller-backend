...
// The counter row lock serializes them: A commits first, B's upsert finds a
...
func TestR19_ConcurrentUpdateSameObject(t *testing.T) {
	...
	w0 := newWriter(t, nil)
	createReq := makeWriteReq("apps/v1/Deployment", "default", "r19", 1)
	...
	// Wait for A to reach BeforeCommit (it holds the counter row lock).
	<-hookA.ready

	// B starts — it will block on the counter row lock until A commits.
	go func() {
		r, e := wB.Write(ctx, updateB)
		chB <- outcome{r, e}
	}()

	// Unblock A — it commits, releasing the counter lock.
	close(hookA.proceed)
	...
	// Counter must be 2 (create=1, A's update=2). B's increment rolled back.
	conn := freshConn(t)
	var counter int64
	...
	require.NoError(t, err)
	assert.Equal(t, int64(2), counter, "B's counter increment must have rolled back")
